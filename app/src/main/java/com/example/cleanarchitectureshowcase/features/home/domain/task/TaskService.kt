...
fun interface TaskAdder { suspend fun add(task: DataTask) }
...
fun interface TaskUpdater { suspend fun update(task: DataTask) }
interface TaskStatusSetter : TaskUpdater { suspend fun setStatus(task: DataTask, isComplete: Boolean) }
